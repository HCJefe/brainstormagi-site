...
// The track is an elevated roller-coaster ribbon that twists, banks, and
// rises through a circuit-board world dotted with futuristic skyscrapers.
// Camera and curve advance with scroll so each section reads as a distinct
// checkpoint with its own building cluster.
(function () {
  "use strict";
  ...
  document.documentElement.dataset.sceneEngine = "canvas2d-roller";
  ...
  // Each section: lateral side target plus a vertical (height) target so
  // the track rises and falls like a roller-coaster, not a flat road.
  var SECTIONS = [
    { id: "hero",        side:  0.00, lift:  0.00, type: "spire" },
    { id: "spire",       side: -0.55, lift:  0.18, type: "spire" },
    { id: "foundry",     side:  0.65, lift: -0.12, type: "foundry" },
    { id: "voice",       side: -0.70, lift:  0.22, type: "voice" },
    { id: "ops",         side:  0.55, lift: -0.10, type: "ops" },
    { id: "revenue",     side: -0.65, lift:  0.16, type: "revenue" },
    { id: "content",     side:  0.70, lift: -0.14, type: "content" },
    { id: "integration", side: -0.55, lift:  0.20, type: "integration" },
  ...
  function seed() {
    ...
    // Electric streaks (elongated tapered light trails) along the track.
    streaks.length = 0;
    for (var p = 0; p < 26; p++) {
      streaks.push({
        z: Math.random(),
        ...
        length: 0.06 + Math.random() * 0.10,
        cmd: Math.random() < 0.18,
  ...
  // Lateral curve target along the track length.
  function curveLateralAt(zNorm, t, sectionPos) {
    ...
    var sway = Math.sin(t * 0.55 + zNorm * 3.2) * 0.20 * (1 - zNorm * 0.4);
    var roll = Math.cos(t * 0.34 + sectionPos * 1.2) * 0.10;
    var blend = 1 - zNorm;
    return fgSide * (1 - blend) + bgSide * blend + sway + roll * blend;
  }

  // Vertical lift along the track length so it climbs and dives like a
  // roller-coaster instead of laying flat. Returns a value in 0..1 of
  // available vertical headroom (positive = lifts the track up the screen).
  function curveLiftAt(zNorm, t, sectionPos) {
    ...
    var bg = liftB + (liftA - liftB) * (1 - ease) * 0.3;
    // Rolling vertical wave gives the track a continuous coaster feel.
    var wave = Math.sin(t * 0.7 + zNorm * 5.0 + sectionPos * 0.8) * 0.10 * (1 - zNorm * 0.3);
    var crest = Math.sin(zNorm * Math.PI * 2 + sectionPos * 1.4 + t * 0.4) * 0.06;
    var blend = 1 - zNorm;
    return fg * (1 - blend) + bg * blend + wave + crest;
  }

  // Project a point on the elevated track ribbon to screen coordinates.
  // zNorm: 0 horizon, 1 camera. laneOffset: -1 left rail, +1 right rail.
  function project(zNorm, laneOffset, w, h, horizonY, t, sectionPos) {
    ...
    // Vertical lift drops the track up the screen for crests and lets it
    // dip back down for valleys, integrated over perspective.
    var lift = curveLiftAt(z, t, sectionPos);
    var groundY = horizonY + (h - horizonY) * pp - lift * (h - horizonY) * 0.45 * pp;

    // Banking from lateral slope.
    var dz = 0.04;
  ...
  // Circuit board substrate. Trace network with pads, vias and chips.
  function drawCircuitBoard(w, h, horizonY, t, sectionPos) {
    ...
    // Solder pads scattered on the board.
  ...
  // Pylons / supports beneath the elevated track. Each pylon drops from
  // the track's center down to the PCB ground plane that would lie at
  // the same z without the vertical lift.
  function drawTrackPylons(w, h, horizonY, t, sectionPos) {
    var COUNT = 14;
    for (var i = 1; i < COUNT; i++) {
      var z = i / COUNT;
      var pCenter = project(z, 0, w, h, horizonY, t, sectionPos);
      // Where the ground would sit at this z (no lift).
      var pp = z * z * 0.94 + z * 0.06;
      ...
      ctx.lineTo(pCenter.x - pylonW / 2, groundY);
      ctx.stroke();
      // Footing pad on the PCB
      ctx.fillStyle = "rgba(255,90,58,0.55)";
      ctx.beginPath();
      ctx.ellipse(pCenter.x, groundY, pylonW * 1.6, pylonW * 0.5, 0, 0, Math.PI * 2);
      ctx.fill();
    }
  }

  // The roller-coaster ribbon itself. Twisting elevated track with rails,
  // cross ties, glowing side conduits, and underglow.
  function drawTrackRibbon(w, h, horizonY, t, sectionPos) {
    var SLICES = 40;

    // Underglow first so the ribbon sits in light.
    for (var u = 0; u < SLICES; u += 2) {
      var uz0 = u / SLICES;
      var uz1 = (u + 2) / SLICES;
      var uL0 = project(uz0, -1.15, w, h, horizonY, t, sectionPos);
      var uR0 = project(uz0,  1.15, w, h, horizonY, t, sectionPos);
      var uL1 = project(uz1, -1.15, w, h, horizonY, t, sectionPos);
      var uR1 = project(uz1,  1.15, w, h, horizonY, t, sectionPos);
    ...
    // Track body slices (trapezoids hugging the spline).
    for (var i = 0; i < SLICES; i++) {
      ...
      var cL0 = project(z0, -0.45, w, h, horizonY, t, sectionPos);
      var cR0 = project(z0,  0.45, w, h, horizonY, t, sectionPos);
      var cL1 = project(z1, -0.45, w, h, horizonY, t, sectionPos);
      var cR1 = project(z1,  0.45, w, h, horizonY, t, sectionPos);
    ...
    // Cross ties: short transverse bars at intervals across the track.
    var TIES = 22;
    for (var ti = 0; ti < TIES; ti++) {
      var tz = (ti / TIES + ((t * 0.20) % (1 / TIES))) % 1;
      if (tz <= 0.02) continue;
      var tL = project(tz, -0.92, w, h, horizonY, t, sectionPos);
      var tR = project(tz,  0.92, w, h, horizonY, t, sectionPos);
      ctx.strokeStyle = "rgba(140,180,220," + (0.3 + tz * 0.5).toFixed(3) + ")";
    ...
    // Side conduits (left cyan, right red) drawn as polylines along curve.
  ...
  // Elongated electric streaks running along the track. Tapered light
  // trails replace the prior dot/ball packets entirely.
  function drawElectricStreaks(w, h, horizonY, t, dt, sectionPos) {
    for (var i = 0; i < streaks.length; i++) {
      ...
      var STEPS = 10;
      var color = sk.cmd ? RED : (sk.lane < 0 ? "#bff8ff" : "#ffe1d0");
      var glow = sk.cmd ? "rgba(255,58,58,0.95)" : "rgba(92,242,255,0.9)";
      ctx.shadowColor = glow;
      ctx.shadowBlur = sk.cmd ? 14 : 10;
      ctx.strokeStyle = color;
      ctx.lineCap = "round";
      ctx.lineJoin = "round";
      // Draw the streak as a tapered polyline so it reads as a light
      // trail, not a chain of beads.
      for (var k = 0; k < STEPS; k++) {
        var u0 = tail + (head - tail) * (k / STEPS);
        var u1 = tail + (head - tail) * ((k + 1) / STEPS);
        if (u0 < 0 || u1 > 1) continue;
        var p0 = project(u0, sk.lane, w, h, horizonY, t, sectionPos);
        var p1 = project(u1, sk.lane, w, h, horizonY, t, sectionPos);
        var taper = (k + 1) / STEPS; // thicker toward head
        var lw = Math.max(0.5, p1.halfW * 0.018 * taper * (sk.cmd ? 1.6 : 1.0));
        var alpha = 0.25 + 0.7 * taper;
        ctx.globalAlpha = alpha;
        ctx.lineWidth = lw;
        ctx.beginPath();
        ctx.moveTo(p0.x, p0.y - lw * 0.4);
        ctx.lineTo(p1.x, p1.y - lw * 0.4);
        ctx.stroke();
      }
      ctx.globalAlpha = 1;
      ctx.shadowBlur = 0;
    ...
    // Sparse arcing sparks that jump from one rail to the other.
    for (var s = 0; s < sparks.length; s++) {
      var sp = sparks[s];
      sp.z += sp.speed * dt;
      if (sp.z > 1) sp.z -= 1;
      if (sp.z < 0.05) continue;
      var phase = (t * 1.6 + sp.offset) % 1;
      if (phase > 0.18) continue; // brief flash window
      var lane = Math.sin(t * 4 + sp.offset) * 0.8;
      var pa = project(sp.z, -0.95, w, h, horizonY, t, sectionPos);
      var pb = project(sp.z,  0.95, w, h, horizonY, t, sectionPos);
      var mx = (pa.x + pb.x) / 2 + Math.cos(t * 6 + sp.offset) * 8;
      var my = (pa.y + pb.y) / 2 - 6 - Math.abs(Math.sin(t * 6 + sp.offset)) * 8;
  ...
  // PCB right-angle branches that peel off the track and run to chip pads.
  ...
  // Detail helpers shared by skyscraper draws.
  ...
  function windowGrid(x, y, w_, h_, cols, rows, t, seed) {
    for (var c = 0; c < cols; c++) {
      for (var r = 0; r < rows; r++) {
        var on = ((Math.sin(t * 2 + r * 1.3 + c * 0.7 + seed) + 1) * 0.5) > 0.45;
  ...
  function drawSpire(p, scale, t) {
    pad(p, scale);
    // Stepped skyscraper: wide base, mid setback, narrow crown, antenna.
    var hgt = scale * 28;
    var basew = scale * 3.4;
    var midw = scale * 2.4;
    var topw = scale * 1.4;
  ...
  function drawFoundry(p, scale, t) {
    pad(p, scale);
    // Industrial cluster: three towers + skybridges + stack vents
    var hgt = scale * 22;
    var towers = [
    ...
    towers.forEach(function (tw, idx) {
      ...
      ctx.stroke();
      // Vent steam
      var puff = (Math.sin(t * 1.2 + idx) + 1) * 0.5;
      ctx.fillStyle = "rgba(255,200,160," + (0.2 + puff * 0.4).toFixed(3) + ")";
      ctx.beginPath();
      ctx.arc(tw.x, p.y - tw.h - scale * 3 - puff * scale * 1.5, scale * (0.4 + puff * 0.3), 0, Math.PI * 2);
      ctx.fill();
    });
    // Skybridges connecting tops
    ctx.strokeStyle = RED;
    ctx.lineWidth = 1.5;
  ...
  function drawVoiceArray(p, scale, t) {
    pad(p, scale);
    // Antenna skyscraper: tall narrow tower with crossbars, dish, wave rings
    var hgt = scale * 26;
    var w_ = scale * 1.6;
    // Tower body
    ctx.fillStyle = "#0a1426";
  ...
  function drawOpsTower(p, scale, t) {
    pad(p, scale);
    // Command skyscraper with observation deck and radar ring
    var hgt = scale * 24;
    var basew = scale * 2.6;
  ...
  function drawReactor(p, scale, t) {
    pad(p, scale);
    // Reactor skyscraper: cylindrical tower with glowing core column,
    // turbine rings around the upper third, and exhaust struts.
    var hgt = scale * 24;
    var w_ = scale * 2.4;
    // Tower body (graphite)
    ...
    // Vertical seam glow strip (the core column visible through the tower)
    ...
    // Turbine rings around the top third
  ...
  function drawContentForge(p, scale, t) {
    pad(p, scale);
    // Studio skyscraper with holographic facade and broadcast crown
    var hgt = scale * 22;
    var basew = scale * 5.0;
    ...
    // Lower window grid (offices below the holo facade)
    var lowerH = hgt * 0.45;
    windowGrid(p.x - basew, p.y - lowerH, basew * 2, lowerH, 10, 4, t, p.x);

    // Holographic facade panel (upper portion)
    ...
    for (var i = 0; i < bars; i++) {
      var bh = (Math.sin(t * 2 + i) + 1) * 0.5 * (facadeH - 4) + 2;
  ...
  function drawIntegrationDome(p, scale, t) {
    pad(p, scale);
    // Dome + tower hybrid: a tall slim tower behind a glowing dome with
    // radial port bridges out to the rails.
    var towerH = scale * 16;
    ...
    for (var k = 0; k < 6; k++) {
      ...
      ctx.beginPath();
      ctx.arc(bx, by, Math.max(1.5, scale * 0.32), 0, Math.PI * 2);
      ctx.fill();
    }
    // Antenna spike on dome
    ctx.strokeStyle = RED;
    ctx.lineWidth = 1;
    ctx.beginPath();
  ...
  function drawContactPad(p, scale, t) {
    pad(p, scale);
    // Landing pad with beacon ring, beacon tower, and four corner light masts
    var padR = scale * 5.0;
    ...
    for (var k = 0; k < 6; k++) {
      ...
      ctx.beginPath();
      ctx.arc(mx, my - scale * 1.7, Math.max(1.0, scale * 0.3), 0, Math.PI * 2);
      ctx.fill();
    }
  }

  // Background skyline kept varied per section so the silhouette feels
  // like a city, not a flat strip.
  ...
  function drawSectionBuildings(w, h, horizonY, t, sectionPos) {
    for (var i = 0; i < SECTIONS.length; i++) {
      ...
      ctx.save();
      ctx.globalAlpha = Math.max(0.25, Math.min(1, 0.4 + z * 0.9));
      switch (sec.type) {
  ...
  function start() {
    ...
    requestAnimationFrame(frame);
    try { console.info("[brainstorm] Canvas2D roller-coaster engine running"); } catch (_) {}
  }
